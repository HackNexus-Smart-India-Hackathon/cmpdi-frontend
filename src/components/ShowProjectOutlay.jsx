...
const ShowProjectOutlay = ({ outlay, name }) => {
  console.log(outlay);
  // const key = outlay.getKey()
  console.log(Object.values(outlay)[0]);
  let outlay1 = Object.values(outlay)[0];
  console.log(name);
  const years =
    Object.keys(outlay1['Captital Expenditure']['Land Building']).length - 1;
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow">
      <h2 className="text-2xl font-bold mb-4">Project Outlay</h2>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border border-gray-300 text-sm">
          ...
          <tbody>
            {Object.entries(outlay).map(([category, subItems]) =>
                ...
                ([subCategory, fields], subIndex) => (
                  <tr key={`${category}-${subCategory}`}>
                    ...
                    <td className="border px-4 py-2">
                      ...
                      /> */}
                      <p>{outlay[category][subCategory]['totalCost']}</p>
                    </td>
                    {fields.yearWise.map((value, yearIndex) => (
                      <td key={yearIndex} className="border px-4 py-2">
                        ...
                        /> */}
                        <p>
                          {outlay[category][subCategory].yearWise.map(
                            (value, index) => (
                              <p key={index}>{value}</p>
                            )
                          )}
...
export default ShowProjectOutlay;
